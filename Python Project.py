# File: project.py
# Author: Fatima Aguado Espinosa and Mia Marche
# Date: 05/09/2022
# Email: fatima123@tamu.edu and Miamarche@tamu.edu
...
def dictionary_of_student_scores():
    ...
    UIN_scores = {}
    for UIN in grades:
        UIN_scores[UIN] = 0.45*sum(grades[UIN][-4:-1])/3 + 0.10*sum(grades[UIN][6:12])/6 + \
                          .10 * sum(grades[UIN][12:18])/6 + .25*sum(grades[UIN][:6])/6 + .1 * grades[UIN][-1]
    return UIN_scores
...
def option_2():
    """This function generates a txt file specific to a certain UIN. This file will contain the student's
...
def option_4():
    """This function gives a letter grade to each student and saves them to a dictionary."""
    # The if statement below checks if the user has done option 1: read the file and if not, it prompts them to do so
    if not grades:
        print("Please read file before proceeding (option 1).")
        return 0
...
# If the user picks 1, the CSV file is read
    if input_choice == '1':
        option_1()
# If the user picks 2 a report for the student is generated
    if input_choice == '2':
        option_2()
# If the user chooses option 3, this creates a bar chart for a student's grades
    if input_choice == '3':
        option_3()
# If the user picks 4, a report for the class is generated
    if input_choice == '4':
        option_4()
# If the user picks 5, a pie and bar chart are generate for the entire class
    if input_choice == '5':
        option_5()
# Once the user types q or quit the program stops
    if input_choice == '6' or input_choice == 'q' or input_choice == 'quit':
        break
